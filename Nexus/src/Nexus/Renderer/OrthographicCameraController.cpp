...
#include "Nexus/Core/KeyCodes.h"

namespace Nexus {

	Nexus::OrthographicCameraController::OrthographicCameraController(float aspectRatio, bool rotation)
	...
	void Nexus::OrthographicCameraController::OnUpdate(Timestep ts)
	{
		...
		if (m_Rotation)
		{
			if (Input::IsKeyPressed(NX_KEY_Q))
			{
				m_CameraRotation += m_CameraRotationSpeed * ts;
			}
			else if (Input::IsKeyPressed(NX_KEY_E))
			{
				m_CameraRotation -= m_CameraRotationSpeed * ts;
			}
